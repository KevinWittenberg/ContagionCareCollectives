---
title: "The Contagion of Citizen Collectives for Care in The Netherlands"
...
path <- "C:/Users/Kevin/ContagionCareCollectives"
...
#Define if the raw administrative data is already loaded in (if you have received the folder with data and already placed it in the relevant directory, otherwise this will attempt to download the data from the internet)
rawdata_loaded <- TRUE
```
...
```{r}
#If you are acquiring the original data files from their respective url's directly, please be sure to check the version of the data.

if (rawdata_loaded == FALSE) {
...
# Checkpoint: The file GR2018_result_total_perc may be loaded from here for inspection and future use
```{r}
...
# Checkpoint: The file df_wijkstats may be loaded from here for inspection and further use
```{r}
...
# Checkpoint: The file df_resid may be loaded from here for inspection and future use
```{r}
#df_resid <- read_feather(file.path(path_clean, "df_resid"))
```

Setting seed again outside of ML pipeline, which was performed on a separate machine
```{r}
set.seed(1337)
```
...
cv_base_f1 <- cv4 - mean(cv_base[1], cv_base[2], cv_base[3])
cv_base_f2 <- cv3 - mean(cv_base[1], cv_base[2], cv_base[4])
cv_base_f3 <- cv2 - mean(cv_base[1], cv_base[3], cv_base[4])
cv_base_f4 <- cv1 - mean(cv_base[2], cv_base[3], cv_base[4])

#Mean accuracy for base model
mean(cv_base_f1, cv_base_f2, cv_base_f3, cv_base_f4)
#standard deviation of baseline model
sd(cv_base_f1, cv_base_f2, cv_base_f3, cv_base_f4)
...
#Evaluate ML performance and variable importances
```{r}
...
saveRDS(MCMI_resid, file.path(path_clean, " MCMI_resid"))
```
...
# Checkpoint: The files df_H1_H2 & df_H3 may be loaded from here for inspection and future use
```{r}
#shape_PC4 <- read_feather(file.path(path_clean, "df_H1_H2"))
#df_local_I_resid <- read_feather(file.path(path_clean, "df_H3"))
```
